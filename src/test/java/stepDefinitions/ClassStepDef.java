...
public class ClassStepDef {
	...
	public void admin_should_see_few_input_fields_and_their_respective_text_boxes_in_the_class_details_window() {
		// assertTrue(classPage.batchNameOnPopupDisp());
		
		/*
		 * Assert.assertTrue(classPage.classTopicOnPopupDisp());
		 * Assert.assertTrue(classPage.classDescriptionOnPopupDisp());
		 * Assert.assertTrue(classPage.commentsonPopupDisp());
		 * Assert.assertTrue(classPage.noOfClassesonPopupDisp());
		 * Assert.assertTrue(classPage.notesPopupDisp());
		 * Assert.assertTrue(classPage.staffNameOnPopupDisp());
		 * Assert.assertTrue(classPage.statusonPopupDisp());
		 * Assert.assertTrue(classPage.recordingonPopupDisp());
		 * Assert.assertTrue(classPage.classDatesonPopupDisp());
		 */
	...
	public void admin_enters_mandatory_fields_in_the_form_and_clicks_on_save_button(String batchName, String classTopic,
		...
		if(classPage.batchnamedropdownDisplayed()) {
			...
			classPage.addingMandatoryFields(batchName.trim(), classTopic.trim(), classDescription.trim(), month.trim(), date.trim(), staffName.trim(), Status.trim());
			Assert.assertTrue(classPage.addingMandatoryFields(batchName.trim(), classTopic.trim(), classDescription.trim(), month.trim(), date.trim(), staffName.trim(), Status.trim())
					.equals(expectedMsg.trim()));
		}
		else System.out.println("Add New Class pop up didnt open");
